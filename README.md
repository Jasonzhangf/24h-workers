...
- **TMUX 注入 + 定时触发**：定时把提示词注入到 tmux 的目标 pane
- **只要是 TTY 工具就能用**：理论上任何 TTY 交互式编程工具都支持
  - 已验证/可用示例：**Claude Code / Codex / iFlow**
...
> ⚠️ **Windows 暂不支持**（依赖 tmux，当前仅支持 macOS / Linux）
...
```bash
npm install -g @jsonstudio/drudge
drudge --version
```
